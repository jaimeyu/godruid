...
const (
	...
	metaViews = `{
		...
		"views": {
			...
			"wordSearch": {
				"map": "function(doc) {\n    if(doc.data.meta) {\n      for (var key in doc.data.meta) {\n        var sentence = doc.data.meta[key];\n        \n        // split on space\n        var words = sentence.split(\" \");\n        \n        for (var word in words) {\n            emit(key + \"__\" + words[word], {\"sentence\":sentence});\n        }\n      }\n    }\n}"
			},
			"allValuesPerKeyPlusPlus": {
				"map": "function(doc) {\n    if(doc.data.meta) {\n      for (var key in doc.data.meta) {\n      \n        emit(key, doc.data.meta[key]);\n        \n      }\n    }\n}",
				"reduce": "function(keys, values, rereduce) {\n    function flatten(arr) {\n      return arr.reduce(function (flat, toFlatten) {\n        return flat.concat(Array.isArray(toFlatten) ? flatten(toFlatten) : toFlatten);\n      }, []);\n    }\n\n    if (rereduce) {\n        var valueCounter = {};\n        var resp = [];\n        resp = values;\n        resp = flatten(resp);\n        var filteredArray = resp.filter(function(item, pos){\n            \n            if (valueCounter[item]) {\n                // Already seen, add 1 more\n                valueCounter[item]++;\n            } else {\n                // Does not previously exist\n                valueCounter[item] = 1;\n            }\n            return resp.indexOf(item)== pos; \n        });\n        \n        if (filteredArray.length > 1000) {\n            return -1;\n        } else {\n            var retDict = {};\n            // Transform the filtered array\n            for (var v in filteredArray) {\n                retDict[filteredArray[v]] = valueCounter[filteredArray[v]];\n            }\n            return retDict;\n        }\n    } else {\n     if (values.length > 1000) {\n        return [];\n     } else {\n        return values;\n     }\n    }\n\n}\n"
			},
			"allValuesPerKeyPlusPlus2": {
				"map": "function(doc) {\n    if(doc.data.meta) {\n      for (var key in doc.data.meta) {\n      \n        emit(key, doc.data.meta[key]);\n        \n      }\n    }\n}",
				"reduce": "function(keys, values, rereduce) {\n    function flatten(arr) {\n      return arr.reduce(function (flat, toFlatten) {\n        return flat.concat(Array.isArray(toFlatten) ? flatten(toFlatten) : toFlatten);\n      }, []);\n    }\n\n    if (rereduce) {\n        var valueCounter = {};\n        var resp = [];\n        resp = values;\n        resp = flatten(resp);\n        var filteredArray = resp.filter(function(item, pos){\n            \n            if (valueCounter[item]) {\n                // Already seen, add 1 more\n                valueCounter[item]++;\n            } else {\n                // Does not previously exist\n                valueCounter[item] = 1;\n            }\n            return resp.indexOf(item)== pos; \n        });\n        \n        if (filteredArray.length > 1000) {\n            return -1;\n        } else {\n            var retDict = [];\n            // Transform the filtered array\n            for (var v in filteredArray) {\n                var set = {\n                           \"type\":filteredArray[v],\n                           \"count\": valueCounter[filteredArray[v]],\n                            };\n                retDict.push(set);\n                //retDict[filteredArray[v]] = valueCounter[filteredArray[v]];\n            }\n            return retDict;\n        }\n    } else {\n     if (values.length > 1000) {\n        return [];\n     } else {\n        return values;\n     }\n    }\n\n}\n"
			},
			"testKeys": {
				"map": "function(doc) {\n    if(doc.data.meta) {\n      for (var key in doc.data.meta) {\n      \n        emit(key, doc.data.meta[key]);\n        \n      }\n    }\n}",
				"reduce": "function(keys, values, rereduce) {\n    const LIMIT = 100;\n    function flatten(arr) {\n      return arr.reduce(function (flat, toFlatten) {\n        return flat.concat(Array.isArray(toFlatten) ? flatten(toFlatten) : toFlatten);\n      }, []);\n    }\n\n    if (rereduce) {           \n        log(\"********* Rereduce XXX\");\n\n        if (values.length < 100) {\n            log(\"********* Rereduce on \" + keys + \" v:\" +values);\n        }\n        var valueCounter = {};\n        var resp = [];\n        resp = values;\n        resp = flatten(resp);\n        var filteredArray = resp.filter(function(item, pos){\n            \n            if (valueCounter[item]) {\n                // Already seen, add 1 more\n                valueCounter[item]++;\n            } else {\n                // Does not previously exist\n                valueCounter[item] = 1;\n            }\n            return resp.indexOf(item)== pos; \n        });\n        \n        if (filteredArray.length > LIMIT) {\n            log(\"filteredArray is too large: \" + filteredArray.length);\n            return -1;\n        } else {\n            var retDict = [];\n            // Transform the filtered array\n            for (var v in filteredArray) {\n                var set = {\n                           \"type\":filteredArray[v],\n                           \"count\": valueCounter[filteredArray[v]],\n                            };\n                if (typeof(set[\"type\"]) != \"object\") {\n                    log(\"Adding Set: {\" + set[\"type\"] + \", \" + set[\"count\"]+ \"}\");\n                    retDict.push(set);\n                    log(\"array len:\" + retDict.length);\n                }\n                //retDict[filteredArray[v]] = valueCounter[filteredArray[v]];\n            }\n            return retDict;\n        }\n    } else {\n     if (values.length > LIMIT) {\n        log(\"***** reduce \" +keys  +\"::values is too large\" + values.length);\n\n        return [];\n     } else {\n        log(\"***** reduce \" +keys + \"::values: \" + values);\n        return values;\n     }\n    }\n\n}\n"
			},
			"lookupWords": {
				"map": "function(doc) {\n    if(doc.data.meta) {\n      for (var key in doc.data.meta) {\n        var sentence = doc.data.meta[key];\n        \n        // split on space\n        var words = sentence.split(\" \");\n        \n        for (var word in words) {\n            emit(key + \"__\" + words[word], {\"sentence\":sentence});\n        }\n      }\n    }\n}",
				"reduce": "function(keys, values, rereduce) {\n    function flatten(arr) {\n      return arr.reduce(function (flat, toFlatten) {\n        return flat.concat(Array.isArray(toFlatten) ? flatten(toFlatten) : toFlatten);\n      }, []);\n    }\n    \n    var sentences = []\n    \n    if (!rereduce) {\n      log(\"reducing:\" + JSON.stringify(keys) + \" || val:\" + JSON.stringify(values))\n      \n      return flatten(values)\n    }\n\n    if (rereduce) {\n      if (!keys) {\n        keys = \"unknown\"\n      }\n      log(\"rereducing:\" + JSON.stringify(keys) + \" || val:\" + JSON.stringify(values))\n\n      var flat = flatten(values)\n      log(\"rereducing:\" + JSON.stringify(keys) + \" || flattened:\" + JSON.stringify(flat))\n    \n      for (var v in flat) {\n        sentences.push(flat[v][\"sentence\"])\n      }      \n\n      var filtered = sentences.filter(function(item, pos){\n            return sentences.indexOf(item)== pos; \n        });\n      \n      var ls = []\n      for (var d in filtered) {\n        var item = {\n          \"sentence\": filtered[d]\n        }\n        ls.push(item)\n      }\n      \n      \n      return ls\n    }\n\n}\n"
			}
		}
	}`
