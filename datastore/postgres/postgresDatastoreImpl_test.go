...
func setupPostgresDB() *TenantMetricBaselinePostgresDAO {
	...
	// Make sure the tables get created for Circle CI testing:
	input, err := ioutil.ReadFile("schema/createGatherDB.sql")
	if err == nil {
		baselineDB.DB.Exec(string(input), nil)
	}
	input, err = ioutil.ReadFile("schema/initMetricBaselines.sql")
	if err == nil {
		baselineDB.DB.Exec(string(input), nil)
	}
