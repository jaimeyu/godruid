...
func RespondWithStructuredErrorList(w http.ResponseWriter, startTime time.Time, code string, objType string, msg tenmod.RequestError, responseCode int) {
	trackAPIMetrics(startTime, code, objType)
	logger.Log.Errorf("%+v", msg)
...
func (tsh *TenantServiceRESTHandler) PatchMonitoredObject(w http.ResponseWriter, r *http.Request) {
	...
	// Issue request to DAO Layer
	oldData, err := tsh.TenantDB.GetMonitoredObject(tenantID, data.ID)
	if err != nil {
...
func (tsh *TenantServiceRESTHandler) UpdateMonitoredObject(w http.ResponseWriter, r *http.Request) {
	...
	err = tsh.TenantDB.UpdateMonitoredObjectMetadataViews(data.TenantID, data.Meta)
	if err != nil {
